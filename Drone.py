...
class Drone():
    def __init__(self, game, id, start_pos, color, icon, explorer):
        ...
        self.graph        = Graph(self.floor_surf, *start_pos, explorer)
    ...
    def move(self, node_id):
        ...
        self.pos = self.explorer.next_pos(self.pos, self.step, self.graph, node_id)
    ...
    def draw_path(self):
        ...
        for i in range(len(self.pos_history)):
            if self.graph.is_free(self.floor_surf):
                pygame.draw.circle(self.floor_surf, (*self.color, 255), self.pos_history[i], 5)
                if i>1 and not self.graph.cross_obs(*(self.pos_history[i]), *(self.pos_history[i-1])):
                    pygame.draw.line(self.floor_surf, (*self.color, 255), self.pos_history[i], self.pos_history[i-1], 2)
